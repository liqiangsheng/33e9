...
>CloudroomQueue 是队列控件，实现队列功能，目的是为了实现用户自动分配。 单例组件，整个程序的生命过程中只能有有一个实例。
...
>操作完成回调[CRVideo_InitQueueDatRslt](#CRVideo_InitQueueDatRslt)，初始化成功后才可获取队列队列相关信息
...
>操作完成则触发[CRVideo_QueueStatusChanged](#CRVideo_QueueStatusChanged)（当前排队的队列，或服务的队列，sdk自动有状态变化回调； 其它队列则需要此函数来查
...
>可以多次调用，开启对多个队列的服务) 操作回调[CRVideo_StartServiceRslt](#CRVideo_StartServiceRslt) 开启成功后： a. 如果没有开启免打挽，那么系统会自动分配客户：VideoCall_Queue_CallBack::autoAssignUser； b. 如果开启免打挽，系统就不会分配客户，如需服务客户可调用：reqAssignUser
