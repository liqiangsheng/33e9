...
var g_chat_id;
var g_isScreenShare = false;//是否屏幕共享
var g_bg_color = 0;//背景色
...
function updateVideo() {
	getVideoList = JSON.parse(CRVideo_GetWatchableVideos());//会议内可观看摄像头列表 
...
function login(){
	if(!g_is_init){
		var result = CRVideo_Init("CLOUDROOM",g_location_dir); //初始化
...
$("#createMeetBtn").click(function(){
	...
	if(m_name != "" || m_name != null){
		$('.loadDiv').css('display','block');
			...
			CRVideo_CreateMeeting(m_name);//创建会议
	}else{
		popTip("会议主题不能为空！");
	}
})
...
$('#nunEnterMeetBtn').click(function(){
	...
	if(g_meet_number != "" || g_meet_number != null){
		$('.loadDiv').css('display','block');
		//进入会议
...
CRVideo_EnterMeetingRslt.callback = function(sdkErr) {
	...
	if(sdkErr == CRVideo_NOERR) {
		...
		if(g_meet_video === undefined) {
			g_meet_video = CRVideo_CreatVideoObj();//视频容器
		...
		if(g_screenShareObj == undefined) {
			g_screenShareObj = CRVideo_CreatScreenShareObj(); //屏幕共享容器
       ...
       //获取当前主功能区 
	   g_current_page = CRVideo_Getcurrentmainpage(); 
	   // 获取视频墙当前分屏模式
	   g_meet_mode = CRVideo_GetVideoWallMode(); 
			if(g_current_page == 0) {
...
$('.meetingDetail_radio_bg').click(function(){
	...
	if(radioVal == "0") {
		CRVideo_Setvideowallmode(2);//2分屏
	} else if(radioVal == "1") {
		CRVideo_Setvideowallmode(3);//4分屏
	} else if(radioVal == "2") {
		CRVideo_Setvideowallmode(5);//6分屏
	}
})
				...
				layer.open({
					...
					success: function(index, layero) {
						...
						$('#screenShareSure').click(function() {
							...
							CRVideo_SetScreenShareCfg(obj);// 设置屏幕共享
						    CRVideo_StartScreenShare(); //开始屏幕共享
							layer.closeAll();
						});
...
function setSure() {
	...
	if($("#mutiVideo").attr("checked") == "checked") {
		g_muti_video = 1;
		CRVideo_SetEnableMutiVideo(g_user_id,g_muti_video);//设置用户是否打开多摄像头
	} else {
		g_muti_video = 0;
		CRVideo_SetEnableMutiVideo(g_user_id,g_muti_video);
	}
...
function setVideoList() {
	...
	var memberList = JSON.parse(CRVideo_GetAllMembers()); //会议成员列表
	var mainListStr = '';
	...
	$('.video_wall_setmainvideo').change(function() {
		if($(this).val() != -1) {
			g_main_id = $(this).val();
			CRVideo_SetMainVideo(g_main_id);//设置主视频
		}
	});
}
...
function setMemberList() {
	var allMemberList = JSON.parse(CRVideo_GetAllMembers());//会议成员列表
	var memberListStr = '';
...
CRVideo_MicEnergyUpdate.callback = function(userID,oldLevel,newLevel){
 	var memberList = $.parseJSON(CRVideo_GetAllMembers());//会议成员列表
...
function videoChange(val,_this) {
	...
	if(g_master == 1) {
		...
		// 	// 没有可打开的音频设备
			...
			// 如果没有禁音
			CRVideo_CloseVideo(val);//关闭摄像头
		    $(_this).children().attr("src", "image/video_close.png");
		}else if(vStatus == 2){
			CRVideo_OpenVideo(val);//开启摄像头
...
function micChange(val,_this) {
	...
	if(g_master == 1) {
		...
		// 	// 没有可打开的音频设备
			...
			// 如果没有禁音
			CRVideo_CloseMic(val);//关闭麦克风
		    $(_this).children().attr("src", "image/mic_close1.png");
		}else if(aStatus == 2){
			CRVideo_OpenMic(val);//开启麦克风
...
function getImgURL(dFile) {
	...
	if(dFile.files && dFile.files[0]) {
        ...
        var fileID = CRVideo_MakeNetDiskFileID(0,fileName);//生成网盘文件ID 
        CRVideo_Uploadnetdiskfile(fileID,$(dFile).val());//上传文件到网盘 
        var fileUploadStr = '';
        	fileUploadStr += '<tr>'
...
$("#refreshBtn").click(function() {
	CRVideo_GetNetDiskFileList();//获取网盘文件列表 
});
//通知网盘上传或下载进度 
...
var sendMsg = function() {
	...
	g_chat_id = CRVideo_SendIMmsg(msg,"","");//发送IM消息
}
// SDK通知使用者发送结果
