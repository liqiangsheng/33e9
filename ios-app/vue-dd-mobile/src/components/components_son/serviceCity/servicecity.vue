...
<script>
  export default{
    ...
    created(){
        let _this=this;
        _this.$axios.post(_this.$addressUrl+_this.$allUrl.cheng_shi+'id=0').then((data)=>{
        if(data.data.code=='0000'){
          if(data.data.result[0].id){   //如果默认第一个为空就给无的提示
            _this.address_region=data.data.result

            _this.$axios.post(_this.$addressUrl+_this.$allUrl.cheng_shi+'id=01').then((data)=>{
              console.log('id=01',data);
              if(data.data.code=='0000'){
    ...
    mounted(){
      let _this=this;
      if(sessionStorage.getItem('dd_city')){//做缓存，每次选择的时候去localStorage里面去查找然后去显示
        let dd_skill=sessionStorage.getItem('dd_city');
    ...
    methods:{
      region_click(item,index){ //点击省份
        ...
        this.$axios.post(_this.$addressUrl + _this.$allUrl.cheng_shi+'id='+item.id).then((data)=>{ //点击哪个省就去请求哪个省下面市的数据
          _this.address_street=data.data.result;
        });
      },
...
//          return false;
        ...
        if(data){
          sessionStorage.setItem('dd_city',JSON.stringify(this.address_header));
